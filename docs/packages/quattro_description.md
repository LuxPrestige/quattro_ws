...
**`quattro_hardware_joint` 매크로**: `hardware_control_method`에 따라 joint당 command interface를 분기한다.

| `hardware_control_method` | command interface |
|---|---|
| `direct_position` | `position` |
| `direct_velocity` | `velocity` |
...
`simulation:=true`/`false` 양쪽 모두 별도로 검사한다. `hardware_control_method`를 바꾸며 4가지 조합도 검사할 수 있다.
