...
`quattro.urdf.xacro`가 `simulation:=true`일 때 `quattro_simulation_joint` 매크로(`gz_ros2_control/GazeboSimSystem`)를 사용하도록 분기한다. 이 매크로는 `quattro_hardware_joint`(실기)와 동일하게 `position` command interface 하나와 `position`/`velocity`/`effort` state interface를 export한다.
