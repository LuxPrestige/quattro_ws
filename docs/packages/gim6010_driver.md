...
- **실기 확인 결과: node 3/5/11이 요청 없이 `Get_Encoder_Estimates(0x09)`를 100Hz로 자발적으로 브로드캐스트한다(2026-08-21, 원인 미확정)**: `candump`로 버스를 관찰한 결과, `front_right_hip_joint`(node 3, `can0`)/`front_right_lower_leg_joint`(node 5, `can0`)/`back_right_lower_leg_joint`(node 11, `can1`) 세 노드는 **아무 프로세스도 실행 중이지 않은 상태에서도** 자기 자신의 encoder estimate 응답(`0x09`)을 대략 10ms 간격(100Hz)으로 계속 내보낸다 — 나머지 9개 노드는 `Heartbeat(0x01)` 100ms 주기 브로드캐스트만 있을 뿐 이런 자발적 전송이 없다. 세 노드에 공통점(같은 관절 종류가 아님 — hip 1개, lower_leg 2개)은 아직 못 찾았다. 이 셋 중 하나(node 5)에서 실제로 런타임 `write()`의 `Set_Input_Pos`가 간헐적으로 거부되는 문제가 재현됐다(`docs/packages/quattro_hardware.md` 2절, "write()의 소비 전략" 참고) — 이 세 노드가 만들어내는 추가 트래픽이 커널 CAN TX 큐(기본 `qlen 10`)를 이따금 채워서 같은 사이클에 다른 명령 전송이 실패하는 것으로 추정된다. 이 자발적 브로드캐스트를 끄는(또는 나머지 9개에 켜는) SDO endpoint를 찾으려 했으나 아래 항목대로 **RxSdo 자체가 이 펌웨어에서 응답하지 않아 endpoint 탐색이 불가능하다.** `docs/development_status.md`에 미해결 항목으로 기록한다. 대신 `QuattroSystem::read()`는 이미 fresh한 feedback을 다시 요청하지 않도록 해서(`docs/packages/quattro_hardware.md` 2절) 이 세 노드에 대한 우리 쪽 중복 요청만이라도 없앴다.
...
| Get_Encoder_Estimates | `0x09` | 요청(RTR)+decode | pos_rev(f32), vel_rev_s(f32) |
